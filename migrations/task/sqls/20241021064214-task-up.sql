...
INSERT INTO
  "COACH"(user_id, experience_years) VALUES (
    (
      SELECT
        id
      FROM
...
UPDATE
  "COURSE_BOOKING"
SET
  cancelled_at = '2024-11-24 17:00:00',status = '已取消'
WHERE
  user_id = (
    SELECT
      id
...
SELECT
  ...
  COUNT("COURSE_BOOKING".join_at) AS total
FROM
  "COURSE_BOOKING"
...
SELECT
  "USER".id,
  COALESCE(SUM(DISTINCT "CREDIT_PURCHASE".purchased_credits), 0) - COALESCE(COUNT(DISTINCT "COURSE_BOOKING".join_at), 0) AS remaining_credit
FROM
  "USER"
  LEFT JOIN "CREDIT_PURCHASE" ON "USER".id = "CREDIT_PURCHASE".user_id
...
WHERE
  EXTRACT(
    MONTH
    FROM
      "COURSE_BOOKING".created_at
  ) = 12
  AND "COURSE_BOOKING".status != '已取消';